...
describe("CurrencyInput", () => {
  it("formats currency text and emits callbacks", async () => {
    ...
    await waitFor(() => expect(onChangeValue).toHaveBeenLastCalledWith(12.34))

    const expectedText = formatCurrency(12.34, { currency: "USD" })
    expect(onChangeText).toHaveBeenLastCalledWith(expectedText)
  ...
  it("toggles negative sign when '-' is typed", async () => {
    ...
    await waitFor(() => expect(onChangeValue).toHaveBeenLastCalledWith(-1.23))
  ...
  it("respects mask='none' and clears to null", async () => {
    ...
    await waitFor(() => expect(onChangeValue).toHaveBeenLastCalledWith(0.99))
    expect(onChangeText).toHaveBeenLastCalledWith("0.99")
